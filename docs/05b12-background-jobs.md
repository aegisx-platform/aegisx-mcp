...
class WorkerManager {
  ...
  private setupGracefulShutdown() {
    const shutdown = async (signal: string) => {
      ...
      const promises = Array.from(this.workers.values()).map(queue => 
        queue.close(5000) // Wait up to 5 seconds for jobs to complete
      );
...
export default fp(async function bullBoardPlugin(fastify: FastifyInstance) {
...
async function workerHealthRoutes(fastify: FastifyInstance) {
  fastify.get('/worker/health', async (request, reply) => {
    ...
    return hasIssues 
      ? reply.error('QUEUE_UNHEALTHY', 'Queue system has issues', 503, health)
  ...
  fastify.post('/worker/clear-failed/:queue', async (request, reply) => {
    ...
    let targetQueue;
    switch (queue) {
      case 'email': targetQueue = emailQueue; break;
      case 'reports': targetQueue = reportQueue; break;
      case 'notifications': targetQueue = notificationQueue; break;
