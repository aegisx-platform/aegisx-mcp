...
export default fp(async function websocketPlugin(fastify: FastifyInstance) {
  ...
  async function handleChatMessage(clientId: string, data: any) {
    ...
    const message = await fastify.knex('chat_messages').insert({
...
export class WebSocketController {
  ...
  async register(fastify: FastifyInstance) {
    fastify.get('/admin/websocket/clients', {
    ...
    fastify.post('/admin/websocket/notify', {
    ...
    fastify.post('/admin/websocket/broadcast', {
    ...
    fastify.delete('/admin/websocket/users/:userId', {
      preHandler: [fastify.authenticate, fastify.authorize(['admin.websocket'])],
      schema: {
        tags: ['WebSocket', 'Admin'],
        params: {
...
export class RoomService {
  ...
  async createRoom(data: CreateRoomRequest, createdBy: string): Promise<ChatRoom> {
    const room = await this.knex('chat_rooms').insert({
      id: this.fastify.generateId(),
      name: data.name,
      type: data.type,
  ...
  async removeMember(roomId: string, userId: string): Promise<void> {
    await this.knex('chat_room_members')
  ...
  recordMessage(type: 'sent' | 'received') {
    this.metricsSignal.update(m => ({
      ...
      [type === 'sent' ? 'messagesSent' : 'messagesReceived']: 
        m[type === 'sent' ? 'messagesSent' : 'messagesReceived'] + 1
    }));
  }
