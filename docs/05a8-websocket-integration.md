...
export class NotificationService {
  ...
  private addNotification(notification: any): void {
    ...
    this.notificationsSignal.update(notifications => 
      [newNotification, ...notifications].slice(0, 100) // Keep last 100
    );
...
export class FileUploadService {
  ...
  async uploadFile(file: File, onProgress?: (progress: number) => void): Promise<UploadedFile> {
    ...
    try {
      ...
      const response = await this.http.post<UploadedFile>('/api/files/upload', formData, {
        reportProgress: true,
        observe: 'events'
      }).pipe(
        tap(event => {
  ...
  private updateProgress(fileId: string, progress: number): void {
    this.uploadProgressSignal.update(map => 
      new Map(map).set(fileId, progress)
    );
  }
