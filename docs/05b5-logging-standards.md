...
export default fp(async function auditLoggerPlugin(fastify: FastifyInstance) {
  ...
  fastify.addHook('onResponse', async (request, reply) => {
    ...
    if (sensitiveActions.includes(request.method) && 
...
export default fp(async function errorLoggerPlugin(fastify: FastifyInstance) {
  ...
  fastify.setErrorHandler(async (error, request, reply) => {
    ...
    if (error.validation) {
      return reply.error(
        ...
        400,
        isDevelopment ? { errorId, details: error.validation } : { errorId }
      );
    }
    ...
    return reply.error(
      ...
      500,
      isDevelopment ? { errorId, stack: error.stack } : { errorId }
    );
  });
...
export default fp(async function performanceLoggerPlugin(fastify: FastifyInstance) {
...
export default fp(async function securityLoggerPlugin(fastify: FastifyInstance) {
...
export default fp(async function correlationIdPlugin(fastify: FastifyInstance) {
...
export default fp(async function monitoringPlugin(fastify: FastifyInstance) {
