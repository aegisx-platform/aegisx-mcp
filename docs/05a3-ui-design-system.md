      ...
      <div class="overflow-x-auto">
        @if (loading()) {
          ...
          <div class="p-8 text-center">
            ...
            <button ui-button variant="outline" (clickEvent)="onRetry()">
              Retry
            </button>
          </div>
        } @else {
...
export class GridComponent {
  ...
  gridClasses = computed(() => {
    ...
    const colClasses = this.responsive() 
      ? `grid-cols-1 md:grid-cols-${this.cols()}`
      : `grid-cols-${this.cols()}`;
      ...
      @for (notification of notifications(); track notification.id) {
        <div [class]="notificationClasses(notification)" @slideInRight>
          <div class="flex">
              ...
              @if (notification.actions) {
                <div class="mt-2 flex space-x-2">
                  @for (action of notification.actions; track action.label) {
                    <button class="text-xs underline opacity-90 hover:opacity-100"
                            (click)="action.action()">
                      {{ action.label }}
                    </button>
                  }
            ...
            <button mat-icon-button (click)="notificationService.remove(notification.id)"
                    class="ml-2 -mr-1">
...
<ui-page-layout [title]="'User Management'" [loading]="userService.loading()">
  ...
  <div slot="header-actions">
    <ui-button variant="primary" [icon]="'add'" (clickEvent)="openCreateDialog()">
      Add User
    </ui-button>
  </div>
  ...
  <ui-grid [cols]="12" gap="lg">
    ...
    <ui-grid-item [span]="12">
      <ui-data-table 
        ...
        <div slot="actions">
          @if (hasSelected()) {
            <ui-button variant="outline" [icon]="'delete'" (clickEvent)="deleteSelected()">
              Delete Selected
            </ui-button>
          }
        </div>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            @for (user of userService.paginatedUsers(); track user.id) {
              <tr class="hover:bg-gray-50">
                ...
                <td class="px-6 py-4">
                  <ui-status-badge [status]="getRoleStatus(user.role)">
                    {{ user.role.name }}
                  </ui-status-badge>
                </td>
                <td class="px-6 py-4">
                ...
                <td class="px-6 py-4">
                  <div class="flex space-x-2">
                    <ui-button size="sm" variant="ghost" [icon]="'edit'" 
                               (clickEvent)="editUser(user)">
                    </ui-button>
                    <ui-button size="sm" variant="ghost" [icon]="'delete'"
                               (clickEvent)="deleteUser(user)">
                    </ui-button>
                  </div>
                </td>
              </tr>
...
<ui-dialog [open]="showDialog()" 
  ...
  <div slot="footer-actions">
    <ui-button variant="outline" (clickEvent)="closeDialog()">
      Cancel
    </ui-button>
...
export class BreakpointService {
  ...
  readonly isTablet = toSignal(
    this.breakpointObserver.observe('(min-width: 768px) and (max-width: 1023px)').pipe(
      map(result => result.matches)
    ),
    { initialValue: false }
  );
