...
export class TenantService {
  ...
  async resolveTenant(request: FastifyRequest): Promise<Tenant | null> {
    ...
    let tenantId = 
      request.headers[this.options.tenantHeader] ||
      this.extractFromSubdomain(request.hostname) ||
      request.query.tenant ||
      this.options.defaultTenant;
    ...
    // Load from database
    const tenant = await this.fastify.knex('tenants')
      .where('slug', tenantId)
...
export abstract class TenantAwareRepository<T, CreateDto, UpdateDto> 
  extends EventDrivenRepository<T, CreateDto, UpdateDto> {
