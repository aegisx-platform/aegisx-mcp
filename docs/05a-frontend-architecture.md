...
export class UserService {
  ...
  async loadUsers(params?: GetUsersParams) {
    ...
    try {
      ...
      const response = await this.http.get<PaginatedUsersResponse>(
        this.baseUrl, 
        { params: httpParams }
      ).toPromise();
  ...
  async createUser(data: CreateUserRequest): Promise<User | null> {
    ...
    try {
      const response = await this.http.post<ApiResponse<User>>(
        this.baseUrl, 
        data
      ).toPromise();
  ...
  async deleteUser(id: string): Promise<boolean> {
    ...
    try {
      const response = await this.http.delete<ApiResponse<{ id: string }>>(
        `${this.baseUrl}/${id}`
      ).toPromise();
