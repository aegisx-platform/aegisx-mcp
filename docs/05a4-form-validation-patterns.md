      ...
      <div class="bg-white p-6 rounded-lg border border-gray-200">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <ui-form-field 
            ...
            [loading]="checkingAvailability().firstName">
            <input matInput 
          ...
          <ui-form-field 
            ...
            [loading]="checkingAvailability().email"
            [hint]="emailHint()"
            class="md:col-span-2">
            <input matInput 
                   type="email"
          ...
          <ui-form-field 
            ...
            [loading]="checkingAvailability().username"
            [hint]="usernameHint()">
            <input matInput 
...
export class DynamicFormComponent implements OnInit {
  ...
  private getFieldsForRole(role: Role): FormField[] {
    ...
    if (role.name === 'Manager') {
      baseFields.push(
        { name: 'department', label: 'Department', type: 'select', required: true, options: this.getDepartmentOptions() },
    ...
    if (role.name === 'Admin') {
      baseFields.push(
        { name: 'accessLevel', label: 'Access Level', type: 'select', required: true, options: this.getAccessLevelOptions() }
      );
    }
      ...
      <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-gray-400 transition-colors">
        @if (uploading()) {
          ...
          <div class="py-4">
            ...
            <button type="button" ui-button variant="outline" size="sm" 
                    (clickEvent)="removeFile()" class="mt-2">
              Remove
            </button>
          </div>
        } @else {
          <div class="py-4">
            ...
            <button type="button" ui-button variant="outline" 
                    (clickEvent)="fileInput.click()" class="mt-2">
...
export class ValidationService {
  ...
  validateEmailAvailability(email: string, excludeUserId?: string): Observable<boolean> {
    ...
    return this.http.post<{ available: boolean }>('/api/validate/email', {
      email,
      excludeUserId
    }).pipe(
  ...
  validateUsernameAvailability(username: string, excludeUserId?: string): Observable<boolean> {
    ...
    return this.http.post<{ available: boolean }>('/api/validate/username', {
      username,
      excludeUserId
    }).pipe(
      map(response => response.available),
...
export class FormTestingUtils {
  ...
  static getErrorMessage(fixture: ComponentFixture<any>, formControlName: string): string | null {
    const errorElement = fixture.debugElement.query(
      By.css(`[data-testid="${formControlName}-error"]`)
    );
