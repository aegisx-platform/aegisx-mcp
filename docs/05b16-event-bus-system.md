...
export class EventBusService {
  ...
  async getEventHistory(aggregateId: string, aggregateType?: string): Promise<DomainEvent[]> {
    const query = this.fastify.knex('domain_events')
...
export class EmailNotificationHandler implements EventHandler {
  ...
  private async sendPaymentReceiptEmail(event: DomainEvent): Promise<void> {
    ...
    try {
      const order = await this.fastify.knex('orders')
        .where('id', orderId)
        .first();
  ...
  async handle(event: DomainEvent): Promise<void> {
    ...
    try {
      ...
      if (event.metadata.userId) {
        await this.fastify.websocketService.sendToUser(
      ...
      if (this.isAdminEvent(event.type)) {
        await this.fastify.websocketService.sendToRoom(
          'admin',
          'admin-notification',
          notification
        );
      ...
      if (this.isSystemEvent(event.type)) {
        await this.fastify.websocketService.broadcast(
          'system-notification',
          notification
        );
      }
...
export class UserService extends BaseService<User, CreateUserRequest, UpdateUserRequest> {
  ...
  async create(data: CreateUserRequest, userId?: string): Promise<User> {
    ...
    const registrationEvent = EventFactory.create(
  ...
  async bulkActivate(userIds: string[], userId?: string): Promise<User[]> {
    ...
    const bulkEvent = EventFactory.create(
      'USERS_BULK_ACTIVATED',
      'bulk-operation',
      'User',
      { userIds, count: results.length },
      { userId }
    );
    ...
    fastify.route({
      ...
      handler: async (request, reply) => {
        ...
        return reply.success(result, 
          `Import completed: ${result.successful.length} successful, ${result.failed.length} failed`
        );
      }
    });
  }
...
export class EventHandlerManager {
  ...
  private registerUserEventHandlers(
    emailHandler: EmailNotificationHandler,
    auditHandler: AuditLogHandler,
    websocketHandler: WebSocketNotificationHandler,
  ...
  private registerBusinessEventHandlers(
    emailHandler: EmailNotificationHandler,
    auditHandler: AuditLogHandler,
    websocketHandler: WebSocketNotificationHandler,
    cacheHandler: CacheInvalidationHandler
...
export class UserRegistrationSaga implements EventHandler {
  ...
  private async handleUserRegistration(event: DomainEvent): Promise<void> {
    ...
    try {
      ...
      const emailEvent = EventFactory.create(
        'SEND_WELCOME_EMAIL',
        user.id,
        'User',
        { user },
      ...
      const onboardingEvent = EventFactory.create(
        'USER_ONBOARDING_STARTED',
        user.id,
        'User',
        { user, step: 'profile_completion' },
        { correlationId: event.id }
      );
      ...
      const failureEvent = EventFactory.create(
  ...
  private async createDefaultUserSettings(userId: string): Promise<void> {
    ...
    const settingsEvent = EventFactory.create(
  ...
  private async handleWelcomeEmailSent(event: DomainEvent): Promise<void> {
    ...
    await this.fastify.knex('user_onboarding')
      .where('user_id', event.aggregateId)
      .update({
...
export class EventProjectionService {
  ...
  async updateUserStatsProjection(event: DomainEvent): Promise<void> {
    ...
    try {
      const currentStats = await this.fastify.knex('event_projections')
        .where({
      ...
      // Upsert projection
      await this.fastify.knex('event_projections')
        .insert({
          projection_name: projectionName,
          aggregate_id: 'global',
          aggregate_type: 'UserStats',
  ...
  async getUserStats(): Promise<any> {
    const projection = await this.fastify.knex('event_projections')
      .where({
...
export abstract class EventSourcedRepository<T> {
  ...
  async loadAggregate(id: string): Promise<T | null> {
    ...
    const events = await this.fastify.knex('domain_events')
      .where('aggregate_id', id)
      .where('aggregate_type', this.getAggregateType())
  ...
  async saveEvents(aggregateId: string, events: DomainEvent[], expectedVersion: number): Promise<void> {
    await this.fastify.knex.transaction(async (trx) => {
      ...
      const lastEvent = await trx('domain_events')
        .where('aggregate_id', aggregateId)
        .where('aggregate_type', this.getAggregateType())
  ...
  private async loadSnapshot(aggregateId: string): Promise<any> {
    return await this.fastify.knex('aggregate_snapshots')
      .where('aggregate_id', aggregateId)
      .where('aggregate_type', this.getAggregateType())
  ...
  async saveSnapshot(aggregateId: string, aggregate: T, version: number): Promise<void> {
    ...
    await this.fastify.knex('aggregate_snapshots')
      .insert({
...
export class EventsController {
  ...
  async register(fastify: FastifyInstance) {
    ...
    fastify.route({
      ...
      preHandler: [fastify.authenticate, fastify.authorize(['admin'])],
      handler: async (request, reply) => {
        const stats = await this.fastify.knex('domain_events')
          .select(
            'type',
            'aggregate_type',
            this.fastify.knex.raw('COUNT(*) as count'),
            this.fastify.knex.raw('MIN(created_at) as first_event'),
            this.fastify.knex.raw('MAX(created_at) as last_event')
          )
          .groupBy('type', 'aggregate_type')
          .orderBy('count', 'desc');
...
describe('Event System E2E', () => {
  ...
  it('should process user creation workflow end-to-end', async () => {
    ...
    const auditLogs = await app.knex('audit_logs')
      .where('resource_id', createdUser.id)
      .where('action', 'CREATE');
    
    expect(auditLogs).toHaveLength(1);

    // Verify welcome email event was triggered
    const emailEvents = await app.knex('domain_events')
      .where('type', 'SEND_WELCOME_EMAIL')
      .where('aggregate_id', createdUser.id);
    
    expect(emailEvents).toHaveLength(1);

    // Verify user settings were created
    const userSettings = await app.knex('user_settings')
...
fastify.route({
  ...
  handler: async (request, reply) => {
    ...
    const eventStats = await fastify.knex('domain_events')
      .select(
        fastify.knex.raw('COUNT(*) as total_events'),
        fastify.knex.raw('COUNT(DISTINCT type) as event_types'),
        fastify.knex.raw('MAX(created_at) as last_event')
      )
      .first();
