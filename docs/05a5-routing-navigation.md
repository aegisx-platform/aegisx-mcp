    ...
    <nav class="bg-white shadow-sm border-r border-gray-200 h-full">
            ...
            <div class="mb-2">
              @if (item.children?.length > 0) {
                ...
                <button 
                  ...
                  class="w-full flex items-center justify-between p-2 text-left rounded-md hover:bg-gray-100 transition-colors">
                  <div class="flex items-center">
                    <mat-icon class="mr-3 text-gray-500">{{ item.icon }}</mat-icon>
                    <span class="text-sm font-medium text-gray-700">{{ item.label }}</span>
                  </div>
                  <mat-icon class="text-gray-400 transition-transform"
                           [class.rotate-180]="isGroupExpanded(item.path)">
                    expand_more
                  </mat-icon>
                </button>
                  ...
                  <div class="ml-6 mt-2 space-y-1">
                    @for (child of item.children; track child.path) {
                      <a [routerLink]="child.path" 
                         routerLinkActive="bg-primary-100 text-primary-700 border-r-2 border-primary-600"
                         class="block p-2 text-sm text-gray-600 hover:text-gray-900 hover:bg-gray-50 rounded-md transition-colors">
                        {{ child.label }}
                      </a>
                    }
                  </div>
      ...
      <div class="absolute bottom-0 left-0 right-0 p-4 border-t border-gray-200">
        <button [matMenuTriggerFor]="userMenu" 
                class="w-full flex items-center p-2 rounded-md hover:bg-gray-100 transition-colors">
      ...
      <nav class="-mb-px flex space-x-8">
        @for (tab of tabs(); track tab.path) {
          <a [routerLink]="tab.path"
             ...
             class="border-b-2 border-transparent py-2 px-1 text-sm font-medium text-gray-500 hover:text-gray-700 hover:border-gray-300 transition-all">
...
@Component({
  template: `
    <ui-page-layout title="User Management">
      <ui-tab-navigation [tabs]="userTabs()">
      </ui-tab-navigation>
    </ui-page-layout>
  `
})
  ...
  template: `
        ...
        <div class="grid grid-cols-4 py-2">
          @for (item of mobileNavItems(); track item.path) {
            <a [routerLink]="item.path"
               ...
               class="flex flex-col items-center py-2 text-gray-500 hover:text-gray-700 transition-colors">
    ...
    @if (breakpointService.isMobile()) {
      <button mat-icon-button 
              ...
              class="fixed top-4 left-4 z-50 bg-white shadow-md">
            ...
            <div class="p-4">
              ...
              @for (item of navigationItems(); track item.path) {
                <a [routerLink]="item.path"
                   ...
                   class="block p-3 rounded-md hover:bg-gray-100 transition-colors">
                  <div class="flex items-center">
...
export class UserListComponent {
  ...
  readonly searchTerm = computed(() => 
    this.queryParamService.getString('search')
  );
