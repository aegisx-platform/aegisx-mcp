...
export class ContractItemService {
  ...
  private baseUrl = '/inventory/master-data/contract-items';

  // ===== SIGNALS FOR STATE MANAGEMENT =====

  private contractItemsListSignal = signal<ContractItem[]>([]);
  private loadingSignal = signal<boolean>(false);
  private errorSignal = signal<string | null>(null);
  private permissionErrorSignal = signal<boolean>(false);
  private lastErrorStatusSignal = signal<number | null>(null);
  private selectedContractItemSignal = signal<ContractItem | null>(null);
  private currentPageSignal = signal<number>(1);
  private pageSizeSignal = signal<number>(10);
  private totalContractItemSignal = signal<number>(0);

  // ===== PUBLIC READONLY SIGNALS =====

  readonly contractItemsList = this.contractItemsListSignal.asReadonly();
  ...
  readonly selectedContractItem = this.selectedContractItemSignal.asReadonly();
  readonly currentPage = this.currentPageSignal.asReadonly();
  readonly totalContractItem = this.totalContractItemSignal.asReadonly();
  readonly pageSize = this.pageSizeSignal.asReadonly();

  // ===== COMPUTED SIGNALS =====

  readonly totalPages = computed(() => {
    const total = this.totalContractItemSignal();
  ...
  /**
   * Load contractItems list with pagination and filters
   */
  async loadContractItemList(params?: ListContractItemQuery): Promise<void> {
    ...
    try {
      ...
      if (params?.contract_id !== undefined)
        httpParams = httpParams.set(
          'contract_id',
          params.contract_id.toString(),
        );
      if (params?.contract_id_min !== undefined)
        httpParams = httpParams.set(
          'contract_id_min',
          params.contract_id_min.toString(),
        );
      if (params?.contract_id_max !== undefined)
        httpParams = httpParams.set(
          'contract_id_max',
          params.contract_id_max.toString(),
        );
      ...
      if (params?.agreed_unit_price !== undefined)
        httpParams = httpParams.set(
          'agreed_unit_price',
          params.agreed_unit_price.toString(),
        );
      if (params?.agreed_unit_price_min !== undefined)
        httpParams = httpParams.set(
          'agreed_unit_price_min',
          params.agreed_unit_price_min.toString(),
        );
      if (params?.agreed_unit_price_max !== undefined)
        httpParams = httpParams.set(
          'agreed_unit_price_max',
          params.agreed_unit_price_max.toString(),
        );
      // Numeric filtering for quantity_limit
      if (params?.quantity_limit !== undefined)
        httpParams = httpParams.set(
          'quantity_limit',
          params.quantity_limit.toString(),
        );
      if (params?.quantity_limit_min !== undefined)
        httpParams = httpParams.set(
          'quantity_limit_min',
          params.quantity_limit_min.toString(),
        );
      if (params?.quantity_limit_max !== undefined)
        httpParams = httpParams.set(
          'quantity_limit_max',
          params.quantity_limit_max.toString(),
        );
      // Numeric filtering for quantity_used
      if (params?.quantity_used !== undefined)
        httpParams = httpParams.set(
          'quantity_used',
          params.quantity_used.toString(),
        );
      if (params?.quantity_used_min !== undefined)
        httpParams = httpParams.set(
          'quantity_used_min',
          params.quantity_used_min.toString(),
        );
      if (params?.quantity_used_max !== undefined)
        httpParams = httpParams.set(
          'quantity_used_max',
          params.quantity_used_max.toString(),
        );
      ...
      const response = await this.http
        .get<
          PaginatedResponse<ContractItem>
        >(this.baseUrl, { params: httpParams })
        .toPromise();

      if (response) {
        this.contractItemsListSignal.set(response.data);

        if (response.pagination) {
          this.totalContractItemSignal.set(response.pagination.total);
          this.currentPageSignal.set(response.pagination.page);
          this.pageSizeSignal.set(response.pagination.limit);
        }
      }
    } catch (error: any) {
      this.handleError(error, 'Failed to load contractItems list');
    } finally {
  ...
  /**
   * Load single contractItems by ID
   */
  async loadContractItemById(id: number): Promise<ContractItem | null> {
    this.loadingSignal.set(true);

    try {
      const response = await this.http
        .get<ApiResponse<ContractItem>>(`${this.baseUrl}/${id}`)
        .toPromise();

      if (response) {
        this.selectedContractItemSignal.set(response.data);
        return response.data;
      }
      return null;
    } catch (error: any) {
      this.handleError(error, 'Failed to load contractItems');
      return null;
    } finally {
      this.loadingSignal.set(false);
    }
   ...
   */
  async createContractItem(
    data: CreateContractItemRequest,
  ): Promise<ContractItem | null> {
    this.loadingSignal.set(true);

    try {
      const response = await this.http
        .post<ApiResponse<ContractItem>>(`${this.baseUrl}`, data)
        .toPromise();
      ...
      this.handleError(error, 'Failed to create contractItems');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  async updateContractItem(
    id: number,
    data: UpdateContractItemRequest,
  ): Promise<ContractItem | null> {
    this.loadingSignal.set(true);

    try {
      const response = await this.http
        .put<ApiResponse<ContractItem>>(`${this.baseUrl}/${id}`, data)
        .toPromise();
      ...
      this.handleError(error, 'Failed to update contractItems');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  async deleteContractItem(id: number): Promise<boolean> {
    ...
    try {
      ...
      this.handleError(error, 'Failed to delete contractItems');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Export contractItems data
   */
  async exportContractItem(options: {
    ...
    try {
      ...
      console.error('Failed to export contractItems data:', error);
      throw error;
    }
  }
  ...
  /**
   * Get dropdown options for contractItems
   */
  async getDropdownOptions(
    ...
    try {
      ...
      console.error('Failed to fetch contractItems dropdown options:', error);
      return [];
    }
  }

  /**
   * Get contracts dropdown options for contract_id field
   */
  async getContractsDropdown(
    params: { search?: string; limit?: number } = {},
    ...
    try {
      ...
      const response = await this.http
        ...
        >('/contracts/dropdown', { params: httpParams })
        .toPromise();
  ...
  /**
   * Bulk create contractItemss
   */
  async bulkCreateContractItem(
    ...
    try {
      ...
      if (response) {
        ...
        await this.loadContractItemList();
        return response;
      }
      return null;
    } catch (error: any) {
      this.handleError(error, 'Failed to bulk create contractItemss');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Bulk update contractItemss
   */
  async bulkUpdateContractItem(
    items: Array<{ id: number; data: UpdateContractItemRequest }>,
  ): Promise<BulkResponse | null> {
    this.loadingSignal.set(true);
    ...
    try {
      ...
      this.handleError(error, 'Failed to bulk update contractItemss');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Bulk delete contractItemss
   */
  async bulkDeleteContractItem(ids: number[]): Promise<BulkResponse | null> {
    this.loadingSignal.set(true);
    ...
    try {
      ...
      this.handleError(error, 'Failed to bulk delete contractItemss');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Validate contractItems data before save
   */
  async validateContractItem(
    data: CreateContractItemRequest,
    ...
    try {
      ...
      console.error('Failed to validate contractItems:', error);
  ...
  async getStats(): Promise<{ total: number } | null> {
    try {
      ...
      console.error('Failed to get contractItems stats:', error);
      return null;
    }
  }
  ...
  /**
   * Select contractItems
   */
  selectContractItem(contractItems: ContractItem | null): void {
    this.selectedContractItemSignal.set(contractItems);
  }
   ...
   */
  reset(): void {
    this.contractItemsListSignal.set([]);
    this.selectedContractItemSignal.set(null);
    this.currentPageSignal.set(1);
    this.errorSignal.set(null);
    this.clearPermissionError();
    this.totalContractItemSignal.set(0);
  }
}
