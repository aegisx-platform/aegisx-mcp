...
@Component({
  selector: 'app-contract-items-import-dialog',
  standalone: true,
  imports: [
    CommonModule,
    MatDialogModule,
    ...
    <div class="tremor-dialog-container">
      ...
      <div class="tremor-dialog-header bg-green-50">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="tremor-dialog-title">Bulk Import Contract Items</h2>
        ...
        @if (currentStep() === 'progress' && importJob()) {
          <div class="step-content">
            <div class="progress-container">
              ...
              <h3>Importing Contract Items...</h3>
...
export class ContractItemImportDialogComponent implements OnDestroy {
  private contractItemsService = inject(ContractItemService);
  private snackBar = inject(MatSnackBar);
  private dialogRef = inject(MatDialogRef<ContractItemImportDialogComponent>);
  ...
  downloadTemplate(format: 'csv' | 'excel'): void {
    this.contractItemsService.downloadImportTemplate(format, true).subscribe({
      next: (blob) => {
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `contract-items-import-template.${format === 'excel' ? 'xlsx' : 'csv'}`;
        link.click();
  ...
  async validateFile(): Promise<void> {
    ...
    try {
      const response = await this.contractItemsService.validateImportFile(file);
  ...
  async executeImport(): Promise<void> {
    ...
    try {
      const response = await this.contractItemsService.executeImport(
  ...
  private startPolling(jobId: string): void {
    this.pollingInterval = setInterval(async () => {
      try {
        const response = await this.contractItemsService.getImportStatus(jobId);
