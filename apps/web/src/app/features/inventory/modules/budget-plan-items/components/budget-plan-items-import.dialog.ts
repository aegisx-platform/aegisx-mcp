...
@Component({
  selector: 'app-budget-plan-items-import-dialog',
  standalone: true,
  imports: [
    CommonModule,
    MatDialogModule,
    ...
    <div class="tremor-dialog-container">
      ...
      <div class="tremor-dialog-header bg-green-50">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="tremor-dialog-title">Bulk Import Budget Plan Items</h2>
              ...
              <div class="preview-section">
                ...
                <div class="table-container">
                  ...
                  >
                    ...
                    <!-- Data Columns -->
                    <ng-container matColumnDef="budget_plan_id">
                      <th mat-header-cell *matHeaderCellDef>Budget Plan Id</th>
                      <td mat-cell *matCellDef="let row">
                        {{ row.budget_plan_id }}
                      </td>
                    </ng-container>
                    ...
                    <ng-container matColumnDef="last_year_qty">
                      <th mat-header-cell *matHeaderCellDef>Last Year Qty</th>
                      <td mat-cell *matCellDef="let row">
                        {{ row.last_year_qty }}
                      </td>
                    </ng-container>
        ...
        @if (currentStep() === 'progress' && importJob()) {
          <div class="step-content">
            <div class="progress-container">
              ...
              <h3>Importing Budget Plan Items...</h3>
...
export class BudgetPlanItemImportDialogComponent implements OnDestroy {
  private budgetPlanItemsService = inject(BudgetPlanItemService);
  private snackBar = inject(MatSnackBar);
  private dialogRef = inject(MatDialogRef<BudgetPlanItemImportDialogComponent>);
  ...
  downloadTemplate(format: 'csv' | 'excel'): void {
    this.budgetPlanItemsService.downloadImportTemplate(format, true).subscribe({
      next: (blob) => {
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `budget-plan-items-import-template.${format === 'excel' ? 'xlsx' : 'csv'}`;
        link.click();
  ...
  async validateFile(): Promise<void> {
    ...
    try {
      const response =
        await this.budgetPlanItemsService.validateImportFile(file);
  ...
  async executeImport(): Promise<void> {
    ...
    try {
      const response = await this.budgetPlanItemsService.executeImport(
  ...
  private startPolling(jobId: string): void {
    this.pollingInterval = setInterval(async () => {
      try {
        const response =
          await this.budgetPlanItemsService.getImportStatus(jobId);
