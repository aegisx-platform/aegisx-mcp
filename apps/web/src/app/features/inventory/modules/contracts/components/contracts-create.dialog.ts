...
import { CreateContractRequest } from '../types/contracts.types';
import {
...
@Component({
  selector: 'app-contracts-create-dialog',
  standalone: true,
  imports: [
    CommonModule,
    MatDialogModule,
  ...
  template: `
    ...
    <h2 mat-dialog-title class="ax-header ax-header-info">
      ...
      <div class="header-text">
        <div class="ax-title">Create New Contract</div>
        <div class="ax-subtitle">Add a new contract to your collection</div>
      </div>
      <button
        type="button"
        mat-icon-button
...
export class ContractCreateDialogComponent {
  ...
  async onFormSubmit(formData: ContractFormData) {
    ...
    try {
      ...
      const createRequest = formData as unknown as CreateContractRequest;
      const result = await this.contractsService.createContract(createRequest);
      ...
      const errorMessage = this.contractsService.permissionError()
        ? 'You do not have permission to create contract'
