...
@Component({
  selector: 'app-contracts-import-dialog',
  standalone: true,
  imports: [
    CommonModule,
    MatDialogModule,
    ...
    <div class="tremor-dialog-container">
      ...
      <div class="tremor-dialog-header bg-green-50">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="tremor-dialog-title">Bulk Import Contracts</h2>
...
export class ContractImportDialogComponent implements OnDestroy {
  private contractsService = inject(ContractService);
  private snackBar = inject(MatSnackBar);
  private dialogRef = inject(MatDialogRef<ContractImportDialogComponent>);
  ...
  downloadTemplate(format: 'csv' | 'excel'): void {
    this.contractsService.downloadImportTemplate(format, true).subscribe({
      next: (blob) => {
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `contracts-import-template.${format === 'excel' ? 'xlsx' : 'csv'}`;
        link.click();
  ...
  async validateFile(): Promise<void> {
    ...
    try {
      const response = await this.contractsService.validateImportFile(file);
  ...
  async executeImport(): Promise<void> {
    ...
    try {
      const response = await this.contractsService.executeImport(
  ...
  private startPolling(jobId: string): void {
    this.pollingInterval = setInterval(async () => {
      try {
        const response = await this.contractsService.getImportStatus(jobId);
