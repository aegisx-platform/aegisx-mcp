...
import { ComprehensiveTest, CreateComprehensiveTestRequest, UpdateComprehensiveTestRequest } from '../types/comprehensive-tests.types';
import { 
  ...
  template: `
    <form [formGroup]="comprehensiveTestsForm" class="-form">
      ...
      <mat-form-field appearance="outline" class="full-width">
        ...
        >
        <mat-error *ngIf="comprehensiveTestsForm.get('title')?.hasError('required')">
          Title is required
        </mat-error>
        <mat-error *ngIf="comprehensiveTestsForm.get('title')?.hasError('maxlength')">
      ...
      <mat-form-field appearance="outline" class="full-width">
        ...
        <textarea 
          ...
          rows="3"
        ></textarea>
        <mat-error *ngIf="comprehensiveTestsForm.get('description')?.hasError('maxlength')">
      ...
      <mat-form-field appearance="outline" class="full-width">
        ...
        <mat-error *ngIf="comprehensiveTestsForm.get('metadata')?.hasError('invalidJson')">
          Invalid JSON format
        </mat-error>
      </mat-form-field>
      ...
      <div class="form-actions">
        ...
        <button 
          ...
          type="button"
          (click)="onSubmit()"
          [disabled]="comprehensiveTestsForm.invalid || loading || (mode === 'edit' && !hasChanges())"
        >
...
export class ComprehensiveTestFormComponent implements OnInit, OnChanges {
  ...
  private populateForm(comprehensiveTests: ComprehensiveTest) {
    const formValue = {
      ...
      expires_at: comprehensiveTests.expires_at ? new Date(comprehensiveTests.expires_at) : null,
      start_time: comprehensiveTests.start_time,
            metadata: comprehensiveTests.metadata ? JSON.stringify(comprehensiveTests.metadata, null, 2) : '',
  ...
  onSubmit() {
    if (this.comprehensiveTestsForm.valid) {
      const formData = { ...this.comprehensiveTestsForm.value } as ComprehensiveTestFormData;
      ...
      if (formData.published_at) {
        formData.published_at = formatDateTimeForSubmission(formData.published_at);
      }
