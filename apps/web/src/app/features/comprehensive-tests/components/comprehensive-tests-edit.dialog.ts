...
import { ComprehensiveTest, UpdateComprehensiveTestRequest } from '../types/comprehensive-tests.types';
import { ComprehensiveTestFormComponent, ComprehensiveTestFormData } from './comprehensive-tests-form.component';
...
export class ComprehensiveTestEditDialogComponent implements OnInit {
  ...
  private dialogRef = inject(MatDialogRef<ComprehensiveTestEditDialogComponent>);
  ...
  async onFormSubmit(formData: ComprehensiveTestFormData) {
    ...
    try {
      ...
      const result = await this.comprehensiveTestsService.updateComprehensiveTest(
        this.data.comprehensiveTests.id, 
        updateRequest
      );
      
      if (result) {
        this.snackBar.open('Comprehensive Tests updated successfully', 'Close', {
          duration: 3000,
        });
