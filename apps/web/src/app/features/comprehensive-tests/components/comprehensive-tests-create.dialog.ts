...
export class ComprehensiveTestCreateDialogComponent {
  ...
  private dialogRef = inject(MatDialogRef<ComprehensiveTestCreateDialogComponent>);
  ...
  async onFormSubmit(formData: ComprehensiveTestFormData) {
    ...
    try {
      ...
      const result = await this.comprehensiveTestsService.createComprehensiveTest(createRequest);
      
      if (result) {
        this.snackBar.open('Comprehensive Tests created successfully', 'Close', {
          duration: 3000,
        });
