...
export async function pdfFontsRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/available', {
            ...
            fonts: {
              ...
              items: {
                ...
                properties: {
                  ...
                  type: { type: 'string', enum: ['builtin', 'custom', 'thai'] }
                }
              }
            }
          }
    ...
    try {
      ...
      const fontsInfo = availableFonts.map(fontName => {
        const isThaiFont = fontManager.isFontAvailable(fontName) && 
  ...
  fastify.get('/status', {
    ...
    try {
      ...
      if (!fontStatus.thaiFontsAvailable) {
        recommendations.push('Download Thai fonts (THSarabun or Sarabun) for better Thai text support');
        recommendations.push('Place font files in assets/fonts directory');
      }
      
      if (fontStatus.failed.length > 0) {
        recommendations.push(`Check font file paths for: ${fontStatus.failed.join(', ')}`);
      }
      
      if (fontStatus.loaded.length === 0) {
        recommendations.push('Using built-in fonts only - consider adding custom fonts for better design flexibility');
      }
  ...
  fastify.post<{
    ...
    try {
      const { fontFamily, text = 'Sample Text สวัสดี ทดสอบฟอนต์ไทย 123', fontSize = 12 } = request.body;
      ...
      const supportsThaiText = /[\u0E00-\u0E7F]/.test(text) && 
                              (fontFamily === 'THSarabun' || fontFamily === 'Sarabun' || fontFamily === 'Helvetica');
  ...
  fastify.get('/recommendations', {
    ...
    try {
      ...
      const recommendations = {
        general: availableFonts.filter(font => ['Helvetica', 'THSarabun'].includes(font)),
        thai: availableFonts.filter(font => ['THSarabun', 'Sarabun'].includes(font)),
        english: availableFonts.filter(font => ['Helvetica', 'Times'].includes(font)),
        monospace: availableFonts.filter(font => ['Courier'].includes(font))
      };
