...
export async function pdfPreviewRoutes(fastify: FastifyInstance) {
  ...
  }, async (request: FastifyRequest<{ Body: Static<typeof PdfPreviewRequestSchema> }>, reply: FastifyReply) => {
    try {
      const { 
        data, 
        fields, 
      ...
      return reply.send({
        ...
        metadata: {
          template,
          pageSize,
          orientation: orientation || (fields && fields.length > 6 ? 'landscape' : 'portrait'),
  ...
  fastify.get<{
    ...
    try {
      ...
      request.log.error({ error, previewId: request.params.previewId }, 'Failed to serve preview file');
  ...
  }, async (request: FastifyRequest<{ Body: Static<typeof PdfTemplateSchema> }>, reply: FastifyReply) => {
    try {
      ...
      const properTemplate: any = {
        ...
        pageMargins: template.pageMargins?.length === 4 ? 
...
function getTemplateDescription(templateName: string): string {
  const descriptions = {
    professional: 'Clean, professional layout with headers, footers, and proper spacing',
    minimal: 'Simple, minimalist design with clean lines and minimal styling',
    standard: 'Standard business format with balanced styling and clear structure'
  };
