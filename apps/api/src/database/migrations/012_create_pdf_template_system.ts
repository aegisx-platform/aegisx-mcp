 ...
 *
 * Creates comprehensive PDF template management with:
 * - Template creation and versioning
 * - Handlebars template support (stored as text, not JSON)
 * - Logo and asset file integration
 * - Sample data and JSON schema validation
 * - Render tracking with performance metrics
 * - Template starter flag for quick duplication
 *
...
export async function up(knex: Knex): Promise<void> {
  // ============================================================================
  // 1. PDF_TEMPLATES TABLE (Main Template Storage)
  // ============================================================================
  await knex.schema.createTable('pdf_templates', (table) => {
    ...
    // Template content (TEXT for Handlebars support, not JSON)
    table.text('template_data').notNullable(); // PDFMake + Handlebars template
    ...
    // Template starter flag (for quick duplication)
    table
      .boolean('is_template_starter')
      .defaultTo(false)
      .notNullable()
      .comment('Flag for template starters that can be duplicated');
    ...
    // File attachments - Assets (array of file IDs)
    table
    ...
    // Legacy assets field (kept for compatibility)
    ...
    // INDEXES
    // ========================================================================
    table.index(['name'], 'idx_pdf_templates_name');
    table.index(['category', 'type'], 'idx_pdf_templates_category_type');
    table.index(['is_active'], 'idx_pdf_templates_is_active');
    table.index(['created_by'], 'idx_pdf_templates_created_by');
  ...
  // ============================================================================
  // 2. PDF_TEMPLATE_VERSIONS TABLE (Version History)
  // ============================================================================
  await knex.schema.createTable('pdf_template_versions', (table) => {
    ...
    // Template content snapshot (TEXT for Handlebars support)
    ...
    // INDEXES
    // ========================================================================
    table.index(['template_id', 'version'], 'idx_pdf_template_versions');
    table.index(['created_at'], 'idx_pdf_template_versions_created');
  });

  // ============================================================================
  // 3. PDF_RENDERS TABLE (Render Tracking)
  // ============================================================================
  await knex.schema.createTable('pdf_renders', (table) => {
    ...
    // INDEXES
    // ========================================================================
    table.index(['template_id'], 'idx_pdf_renders_template');
    table.index(['rendered_by'], 'idx_pdf_renders_rendered_by');
    table.index(['rendered_at'], 'idx_pdf_renders_rendered_at');
    table.index(['status'], 'idx_pdf_renders_status');
  });
  ...
  await knex('pdf_templates').insert([
    {
      ...
      sample_data: {
        ...
        items: [
          ...
          { description: 'Item 2', quantity: 1, price: 250.0 },
    ...
    },
    {
      ...
      description:
        'Professional invoice template with company details and itemized billing',
      category: 'financial',
      type: 'invoice',
      template_data: JSON.stringify({
        pageSize: 'A4',
