...
export class TestProductsImportService extends BaseImportService<TestProducts> {
  ...
  private static createConfig(
    ...
    const fields: ImportFieldConfig[] = [
      ...
      },
      {
        ...
        validators: [
          TestProductsImportService.validateSlugUniqueness(
            testProductsRepository,
          ),
        ],
        ...
        errorMessages: {
          unique:
            TestProductsErrorMessages[TestProductsErrorCode.DUPLICATE_SLUG],
        },
      },
      {
        name: 'description',
  ...
  /**
   * Create slug uniqueness validator
   */
  private static validateSlugUniqueness(
    repository: TestProductsRepository,
  ): (
    value: any,
    row: any,
    ...
    return async (value: any, _row: any, _index: number) => {
      ...
      try {
        const existing = await repository.findBySlug(value);
        if (existing) {
          return {
            field: 'slug',
            message:
              TestProductsErrorMessages[TestProductsErrorCode.DUPLICATE_SLUG],
            code: TestProductsErrorCode.DUPLICATE_SLUG,
            severity: ImportValidationSeverity.ERROR,
            value,
          };
        }
