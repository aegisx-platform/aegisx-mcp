...
- **Data Flow:** Database: users → user_roles → roles → role_permissions → permissions | Cache: Redis stores user permissions | Permission format: 'users:read', 'users:create' | Admin wildcard: '*:*' | Protected routes: users, departments, navigation, rbac, settings, monitoring, file-upload, pdf-export, file-audit, login-attempts
