...
## Phase 2: Update CRUD Generator (Week 3)

- [ ] 2.1. Add layer classification logic to generator
  - File: `libs/aegisx-cli/src/utils/layer-classifier.ts` (NEW)
  - Implement `determineLayer(domain, type)` function
  - Add decision tree logic for Core/Platform/Domains classification
  - Include validation and error handling
  - _Leverage: `docs/architecture/api-standards/02-architecture-specification.md` (Module Categorization Rules)_
  - _Requirements: 2, 6_
  - _Prompt: Implement the task for spec api-architecture-standardization, first run spec-workflow-guide to get the workflow guide then implement the task: Role: TypeScript Developer with expertise in code generation and classification logic | Task: Create layer classification utility following the module categorization rules from the architecture specification, implementing determineLayer() function with comprehensive decision tree logic | Restrictions: Must handle all edge cases, provide clear error messages for ambiguous cases, follow TypeScript best practices | Success: Function correctly classifies modules into Core/Platform/Domains layers, handles edge cases gracefully, includes unit tests with 100% coverage | Instructions: 1) Mark in-progress, 2) Log with artifacts (function signature, test cases, classification examples), 3) Mark complete_

- [ ] 2.2. Update backend plugin template
  ...
  - _Leverage: `docs/architecture/api-standards/03-plugin-pattern-specification.md`, `apps/api/src/core/auth/auth.plugin.ts`_
  - _Requirements: 3, 6_
  - _Prompt: Implement the task for spec api-architecture-standardization, first run spec-workflow-guide to get the workflow guide then implement the task: Role: Template Engineer with expertise in EJS and Fastify plugin patterns | Task: Update plugin template following the plugin pattern specification, adding conditional fp() wrapper logic and updating directory structure to layers-based approach | Restrictions: Must maintain backward compatibility, generate valid TypeScript code, follow template best practices | Success: Template correctly generates fp() wrapper for infrastructure/aggregators, plain async for leaf modules, code compiles without errors | Instructions: 1) Mark in-progress, 2) Log with artifacts (template diff, generated code examples), 3) Mark complete_

- [ ] 2.3. Update backend routes template
  ...
  - _Leverage: `docs/architecture/api-standards/04-url-routing-specification.md`_
  - _Requirements: 4, 6_
  - _Prompt: Implement the task for spec api-architecture-standardization, first run spec-workflow-guide to get the workflow guide then implement the task: Role: Backend Developer with Fastify routing expertise | Task: Update routes template following URL routing specification, changing prefix pattern to /api/v1/{layer}/{domain}/{resource} and ensuring proper route registration | Restrictions: Must maintain route object format, preserve schema validation, follow Fastify routing best practices | Success: Generated routes use correct URL pattern, schema validation is properly configured, routes register correctly | Instructions: 1) Mark in-progress, 2) Log with artifacts (route examples, URL patterns), 3) Mark complete_

- [ ] 2.4. Update generator main logic
...
- [ ] 2.5. Update generator templates (controller, service, repository)
  ...
  - _Leverage: Existing templates, `docs/architecture/api-standards/02-architecture-specification.md`_
  - _Requirements: 6_
  - _Prompt: Implement the task for spec api-architecture-standardization, first run spec-workflow-guide to get the workflow guide then implement the task: Role: Full-stack Developer with code generation expertise | Task: Update controller, service, and repository templates to work with layer-based directory structure, fixing import paths and maintaining existing architectural patterns | Restrictions: Must preserve Repository→Service→Controller pattern, maintain type safety, follow existing conventions | Success: Generated code has correct import paths, architectural pattern is maintained, code compiles and passes linting | Instructions: 1) Mark in-progress, 2) Log with artifacts (import path changes, template updates), 3) Mark complete_

- [ ] 2.6. Test generator with sample tables
  ...
  - Verify generated code compiles and passes linting
  - _Leverage: `libs/aegisx-cli/__tests__/`_
  - _Requirements: 6_
  - _Prompt: Implement the task for spec api-architecture-standardization, first run spec-workflow-guide to get the workflow guide then implement the task: Role: QA Engineer with expertise in code generation testing | Task: Create comprehensive test suite for updated generator, testing all three layers with sample tables and verifying generated code quality | Restrictions: Must test all code paths, verify TypeScript compilation, ensure generated code follows standards | Success: All tests pass, generated code compiles without errors, linting passes, code follows architectural standards | Instructions: 1) Mark in-progress, 2) Log with artifacts (test cases, generated code samples), 3) Mark complete_

- [ ] 2.7. Update generator documentation
  ...
  - _Leverage: `docs/architecture/api-standards/05-crud-generator-specification.md`_
  - _Requirements: 6, 7_
  - _Prompt: Implement the task for spec api-architecture-standardization, first run spec-workflow-guide to get the workflow guide then implement the task: Role: Technical Writer with developer tools documentation expertise | Task: Update CRUD generator documentation to reflect layer-based changes, documenting --layer option, classification logic, and providing clear examples | Restrictions: Must be clear and concise, include practical examples, maintain existing documentation structure | Success: Documentation is complete and accurate, examples work as shown, developers can use generator without confusion | Instructions: 1) Mark in-progress, 2) Log with artifacts (documentation sections, examples), 3) Mark complete_

## Phase 3: Setup & Route Aliasing (Week 4)

- [ ] 3.1. Create layer directory structure
...
- [ ] 3.2. Implement route aliasing plugin
...
- [ ] 3.3. Add feature flags to configuration
...
- [ ] 3.4. Create layer-based plugin groups in plugin loader
...
- [ ] 3.5. Register route aliasing plugin in loader
  - File: `apps/api/src/bootstrap/plugin.loader.ts`
...
- [ ] 3.6. Create comprehensive tests for route aliasing
...
- [ ] 3.7. Deploy to staging with feature flags disabled
...
- [ ] 3.8. Enable new routes in staging and test
...
## Phase 4: Module Migration (Weeks 5-7)

### Batch 1: Low-Risk Platform Services (Week 5)

- [ ] 4.1. Migrate departments module to Platform layer
...
- [ ] 4.2. Migrate settings module to Platform layer
  ...
  - _Leverage: Pattern from 4.1 (departments migration)_
  - _Requirements: 1, 3, 8_
  - _Prompt: Implement the task for spec api-architecture-standardization, first run spec-workflow-guide to get the workflow guide then implement the task: Role: Backend Developer with migration expertise | Task: Migrate settings module to Platform layer following the same pattern used for departments (task 4.1) | Restrictions: Same as 4.1 - maintain functionality, dual registration, thorough testing | Success: Settings module migrated successfully, both routes work, no regressions | Instructions: 1) Mark in-progress, 2) Log with artifacts (migration details, tests), 3) Mark complete_

- [ ] 4.3. Migrate navigation module to Platform layer
...
- [ ] 4.4. Test Batch 1 migrations in staging
...
### Batch 2: Medium-Risk Platform Services (Week 6)

- [ ] 4.5. Migrate users module to Platform layer
...
- [ ] 4.6. Migrate rbac module to Platform layer
  ...
  - _Leverage: Pattern from 4.5 (users migration)_
  - _Requirements: 1, 3, 8_
...
- [ ] 4.7. Migrate file-upload and attachments modules to Platform layer
...
- [ ] 4.8. Migrate pdf-export and import modules to Platform layer
  - Copy both modules to `apps/api/src/layers/platform/`
...
- [ ] 4.9. Test Batch 2 migrations in staging
...
### Batch 3: High-Risk Core Infrastructure (Week 7)

- [ ] 4.10. Migrate monitoring module to Core layer
  ...
  - _Leverage: `docs/architecture/api-standards/03-plugin-pattern-specification.md`_
  - _Requirements: 1, 3, 8_
...
- [ ] 4.11. Migrate auth and audit modules to Core layer
...
- [ ] 4.12. Migrate inventory and admin domains
  ...
  - _Leverage: Existing domain structure_
  - _Requirements: 1, 3, 8_
...
- [ ] 5.2. Run integration tests for all layers
...
- [ ] 5.3. Execute E2E tests for critical user journeys
...
- [ ] 5.4. Conduct performance benchmarking
...
## Phase 6: Production Deployment & Cleanup (Weeks 9-10)

- [ ] 6.1. Deploy to production with feature flags
...
- [ ] 6.2. Enable new routes in production
...
- [ ] 6.3. Add deprecation headers to old routes
...
- [ ] 6.4. Monitor and track old route usage
...
- [ ] 6.5. Disable old routes after sunset period
  ...
  - _Leverage: Migration tracking from 6.4_
  - _Requirements: 5, 9_
...
- [ ] 6.6. Remove old code and route aliasing
...
- [ ] 6.7. Archive migration artifacts and document
...
- Phase 2 (CRUD Generator): 7 tasks
- Phase 3 (Setup): 8 tasks
- Phase 4 (Migration): 13 tasks (4 batches)
